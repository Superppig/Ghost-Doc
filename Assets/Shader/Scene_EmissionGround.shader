...
Shader "XinY/Scene_EmissionGround"
{
    Properties
    {
        ...
        _FlowWidth("FlowWidth",Range(0,0.5))=0.4
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert(appdata v)
            {
                ...
                //UV相关
                output.uv = float4(TRANSFORM_TEX(v.texcoord, _NormalMap), v.texcoord);
            ...
            float4 frag(v2f i) : SV_TARGET
            {
                ...
                half2 emissUV = i.uv.xy;
                half emissMap = SAMPLE_TEXTURE2D(_EmissionMap, sampler_EmissionMap, emissUV);
                half disFactor=2*(frac(_Time.y*_FlowSpeed)-0.5);
                half emissMask = smoothstep(0.5-_FlowWidth, 0.5, 0.5 - distance(i.uv.w,disFactor));
                ...
                half3 emission = _EmissionColor * emissMask*emissMap;
        ...
        Pass
        {
            ...
            {
                half4 baseMap = _BaseColor;
                half4 MRHA = SAMPLE_TEXTURE2D(_MRHA, sampler_MRHA, input.uv);
