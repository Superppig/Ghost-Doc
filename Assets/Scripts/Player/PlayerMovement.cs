...
public class PlayerMovement : MonoBehaviour
{
    ...
    [Header("跳跃")] public float jumpForce; //跳跃时给向上的瞬间力
    ...
    private void MovePlayer()
    {
        ...
        {
            rb.AddForce(GetSlopeMoveDir(moveDirection) * (moveSpeed * VerForce),
                ForceMode.Force); //在斜坡上作用力方向为输入方向在斜坡上的投影
        }
        else
    ...
    {
        wallRight = Physics.Raycast(transform.position, orientation.right, out wallRightHit, wallCheckDistance,
            whatIsWall);
        wallLeft = Physics.Raycast(transform.position, -orientation.right, out wallLeftHit, wallCheckDistance,
            whatIsWall);
    }
